...
GRAPH * read(GRAPH * G, char * filename){
  ...
  //freeup memory which is not required anymore
  for (int i = 0; i < num_actual_vertices; i++){
    free(cp_labels[i]);
    cp_labels[i] = NULL;
  }
...
HEAP * heap_insert(HEAP * heap,int vertex,int dist){
  if(heap->number_of_elements==heap->size){
    printf("Heap is already full!\n");
    return heap;
  }

  int i = (heap->number_of_elements)++;
  int parent = (i-1)/2;
  while(i && dist < heap->Array[parent]->d){
    heap->Array[i] = heap->Array[parent];
    i = parent;
    parent = (i-1)/2;
  }

  heap->Array[i] = create_heap_node(vertex,dist,0);
  return heap;
}
...
HEAP * decrease_key(HEAP * heap,int pre_vertex, int vertex,int distance){
  ...
  if(distance < heap->Array[idx]->d){
    ...
    while(heap->Array[parent]->d > heap->Array[idx]->d) {
        //swapping after inserting 
        swap_nodes(&(heap->Array[parent]),&(heap->Array[idx]));
        idx = parent;
        parent = (idx-1)/2;
    }
    int temp_idx = getHeapIdx(heap,vertex);
  }
  return heap;
}
...
void dijkstra(GRAPH * G, int destination_vertex){
  ...
  HEAP * heap = make_heap(G,destination_vertex);
  while(!isEmpty(heap)){
